...
define(['exports', './when-45f3d25d', './Check-34538dad', './Cartesian3-e69091b9', './Cartesian2-7a44370a', './BoundingSphere-bacc5cb6', './Matrix4-c65e6a1b', './Rectangle-c7d55cfa', './ComponentDatatype-86703c58', './GeometryAttribute-70b679fd', './PrimitiveType-30fa6f85', './Transforms-7d49a8ab', './GeometryAttributes-9d45f9e2', './GeometryPipeline-57d51dc8', './IndexDatatype-e52361bd', './GeometryOffsetAttribute-f755a5cb', './VertexFormat-a00562ee', './EllipseGeometryLibrary-60c39a5b', './GeometryInstance-734856d8'], function (exports, when, Check, Cartesian3, Cartesian2, BoundingSphere, Matrix4, Rectangle, ComponentDatatype, GeometryAttribute, PrimitiveType, Transforms, GeometryAttributes, GeometryPipeline, IndexDatatype, GeometryOffsetAttribute, VertexFormat, EllipseGeometryLibrary, GeometryInstance) { 'use strict';
    ...
    var scratchCartesian1 = new Cartesian3.Cartesian3();
        ...
        var textureMatrixScratch = new Matrix4.Matrix3();
        var tangentMatrixScratch = new Matrix4.Matrix3();
        var quaternionScratch = new Transforms.Quaternion();

        var scratchNormal = new Cartesian3.Cartesian3();
        var scratchTangent = new Cartesian3.Cartesian3();
        var scratchBitangent = new Cartesian3.Cartesian3();

        var scratchCartographic = new Cartesian2.Cartographic();
        var projectedCenterScratch = new Cartesian3.Cartesian3();
        ...
        function computeTopBottomAttributes(positions, options, extrude) {
            ...
            if (stRotation !== 0) {
                ...
                textureMatrix = Matrix4.Matrix3.fromQuaternion(rotation, textureMatrix);

                rotation = Transforms.Quaternion.fromAxisAngle(geodeticNormal, -stRotation, quaternionScratch);
                tangentMatrix = Matrix4.Matrix3.fromQuaternion(rotation, tangentMatrix);
            } else {
                textureMatrix = Matrix4.Matrix3.clone(Matrix4.Matrix3.IDENTITY, textureMatrix);
                tangentMatrix = Matrix4.Matrix3.clone(Matrix4.Matrix3.IDENTITY, tangentMatrix);
            }
            ...
            for (var i = 0; i < length; i += 3) {
                var i1 = i + 1;
                var i2 = i + 2;
                var position = Cartesian3.Cartesian3.fromArray(positions, i, scratchCartesian1);

                if (vertexFormat.st) {
                    var rotatedPoint = Matrix4.Matrix3.multiplyByVector(textureMatrix, position, scratchCartesian2);
                ...
                if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent || shadowVolume) {
                    ...
                    if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
                        if (vertexFormat.tangent || vertexFormat.bitangent) {
                            tangent = Cartesian3.Cartesian3.normalize(Cartesian3.Cartesian3.cross(Cartesian3.Cartesian3.UNIT_Z, normal, tangent), tangent);
                            Matrix4.Matrix3.multiplyByVector(tangentMatrix, tangent, tangent);
                        }
                        ...
                        if (vertexFormat.bitangent) {
                            bitangent = Cartesian3.Cartesian3.normalize(Cartesian3.Cartesian3.cross(normal, tangent, bitangent), bitangent);
        ...
        var boundingSphereCenter = new Cartesian3.Cartesian3();

        function computeEllipse(options) {
            var center = options.center;
            boundingSphereCenter = Cartesian3.Cartesian3.multiplyByScalar(options.ellipsoid.geodeticSurfaceNormal(center, boundingSphereCenter), options.height, boundingSphereCenter);
            boundingSphereCenter = Cartesian3.Cartesian3.add(center, boundingSphereCenter, boundingSphereCenter);
        ...
        function computeWallAttributes(positions, options) {
            ...
            ellipsoid.geodeticSurfaceNormal(geodeticNormal, geodeticNormal);
            var rotation = Transforms.Quaternion.fromAxisAngle(geodeticNormal, stRotation, quaternionScratch);
            var textureMatrix = Matrix4.Matrix3.fromQuaternion(rotation, textureMatrixScratch);
            ...
            for (var i = 0; i < length; i += 3) {
                ...
                var scaledNormal = Cartesian3.Cartesian3.multiplyByScalar(normal, height, scratchCartesian4);
                position = Cartesian3.Cartesian3.add(position, scaledNormal, position);
                scaledNormal = Cartesian3.Cartesian3.multiplyByScalar(normal, extrudedHeight, scaledNormal);
                extrudedPosition = Cartesian3.Cartesian3.add(extrudedPosition, scaledNormal, extrudedPosition);
                ...
                if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
                    ...
                    var next = Cartesian3.Cartesian3.fromArray(positions, (i + 3) % length, scratchCartesian4);
                    Cartesian3.Cartesian3.subtract(next, position, next);
                    var bottom = Cartesian3.Cartesian3.subtract(extrudedPosition, position, scratchCartesian3);

                    normal = Cartesian3.Cartesian3.normalize(Cartesian3.Cartesian3.cross(bottom, next, normal), normal);
        ...
        function computeExtrudedEllipse(options) {
            ...
            var ellipsoid = options.ellipsoid;
            var semiMajorAxis = options.semiMajorAxis;
            var scaledNormal = Cartesian3.Cartesian3.multiplyByScalar(ellipsoid.geodeticSurfaceNormal(center, scratchCartesian1), options.height, scratchCartesian1);
            topBoundingSphere.center = Cartesian3.Cartesian3.add(center, scaledNormal, topBoundingSphere.center);
            topBoundingSphere.radius = semiMajorAxis;

            scaledNormal = Cartesian3.Cartesian3.multiplyByScalar(ellipsoid.geodeticSurfaceNormal(center, scaledNormal), options.extrudedHeight, scaledNormal);
            bottomBoundingSphere.center = Cartesian3.Cartesian3.add(center, scaledNormal, bottomBoundingSphere.center);
        ...
        function computeRectangle(center, semiMajorAxis, semiMinorAxis, rotation, granularity, ellipsoid, result) {
            ...
            var rectangle = Rectangle.Rectangle.fromCartesianArray(positions, ellipsoid, result);
            // Rectangle width goes beyond 180 degrees when the ellipse crosses a pole.
            // When this happens, make the rectangle into a "circle" around the pole
            if (rectangle.width > Cartesian3.CesiumMath.PI) {
                rectangle.north = rectangle.north > 0.0 ? Cartesian3.CesiumMath.PI_OVER_TWO - Cartesian3.CesiumMath.EPSILON7 : rectangle.north;
                rectangle.south = rectangle.south < 0.0 ? Cartesian3.CesiumMath.EPSILON7 - Cartesian3.CesiumMath.PI_OVER_TWO : rectangle.south;
                rectangle.east = Cartesian3.CesiumMath.PI;
                rectangle.west = -Cartesian3.CesiumMath.PI;
            }
            return rectangle;
        }
        ...
        function EllipseGeometry(options) {
            ...
            var semiMajorAxis = options.semiMajorAxis;
            var semiMinorAxis = options.semiMinorAxis;
            var granularity = when.defaultValue(options.granularity, Cartesian3.CesiumMath.RADIANS_PER_DEGREE);
         ...
         * @type {Number}
         */
        EllipseGeometry.packedLength = Cartesian3.Cartesian3.packedLength + Cartesian2.Ellipsoid.packedLength + VertexFormat.VertexFormat.packedLength + 9;
        ...
        EllipseGeometry.unpack = function(array, startingIndex, result) {
            ...
            var center = Cartesian3.Cartesian3.unpack(array, startingIndex, scratchCenter);
            startingIndex += Cartesian3.Cartesian3.packedLength;
...
//# sourceMappingURL=EllipseGeometry-f4a055ac.js.map
