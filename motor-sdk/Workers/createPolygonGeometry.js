...
define(['./when-45f3d25d', './Check-34538dad', './Cartesian3-e69091b9', './Cartesian2-7a44370a', './BoundingSphere-bacc5cb6', './Matrix4-c65e6a1b', './RuntimeError-86da6af2', './Rectangle-c7d55cfa', './WebGLConstants-3660bc8f', './ComponentDatatype-86703c58', './GeometryAttribute-70b679fd', './PrimitiveType-30fa6f85', './Transforms-7d49a8ab', './GeometryAttributes-9d45f9e2', './AttributeCompression-1e177d5e', './GeometryPipeline-57d51dc8', './EncodedCartesian3-7b803d4a', './IndexDatatype-e52361bd', './IntersectionTests-8855da40', './Plane-1875afb4', './GeometryOffsetAttribute-f755a5cb', './VertexFormat-a00562ee', './GeometryInstance-734856d8', './arrayRemoveDuplicates-24a803e7', './BoundingRectangle-c7afec11', './EllipsoidTangentPlane-3e14dd4d', './ArcType-39be7a32', './EllipsoidRhumbLine-4bc7760a', './PolygonPipeline-d22f3330', './PolygonGeometryLibrary-c7aa9cd4', './EllipsoidGeodesic-1d7258d7'], function (when, Check, Cartesian3, Cartesian2, BoundingSphere, Matrix4, RuntimeError, Rectangle, WebGLConstants, ComponentDatatype, GeometryAttribute, PrimitiveType, Transforms, GeometryAttributes, AttributeCompression, GeometryPipeline, EncodedCartesian3, IndexDatatype, IntersectionTests, Plane, GeometryOffsetAttribute, VertexFormat, GeometryInstance, arrayRemoveDuplicates, BoundingRectangle, EllipsoidTangentPlane, ArcType, EllipsoidRhumbLine, PolygonPipeline, PolygonGeometryLibrary, EllipsoidGeodesic) { 'use strict';
    ...
    var scratchPerPosNormal = new Cartesian3.Cartesian3();
    var scratchPerPosTangent = new Cartesian3.Cartesian3();
    var scratchPerPosBitangent = new Cartesian3.Cartesian3();
    ...
    var appendTextureCoordinatesCartesian3 = new Cartesian3.Cartesian3();
    var appendTextureCoordinatesQuaternion = new Transforms.Quaternion();
    var appendTextureCoordinatesMatrix3 = new Matrix4.Matrix3();
    ...
    function computeAttributes(options) {
        ...
        if (
            ...
            if (stRotation !== 0.0) {
                ...
                tangentRotationMatrix = Matrix4.Matrix3.fromQuaternion(
                    rotation,
                    tangentRotationMatrix
                );
            } else {
                textureMatrix = Matrix4.Matrix3.clone(Matrix4.Matrix3.IDENTITY, textureMatrix);
                tangentRotationMatrix = Matrix4.Matrix3.clone(
            ...
            for (var i = 0; i < length; i += 3) {
                var position = Cartesian3.Cartesian3.fromArray(
                    flatPositions,
                    i,
                ...
                if (vertexFormat.st) {
                    var p = Matrix4.Matrix3.multiplyByVector(
                        textureMatrix,
                        position,
                        scratchPosition
                    );
                    ...
                    var stx = Cartesian3.CesiumMath.clamp(
                        st.x / boundingRectangle.width,
                        0,
                        1
                    );
                    var sty = Cartesian3.CesiumMath.clamp(
                ...
                if (
                    ...
                    if (wall) {
                        if (i + 3 < length) {
                            var p1 = Cartesian3.Cartesian3.fromArray(
                                flatPositions,
                                i + 3,
                                p1Scratch
                            );
                            ...
                            if (recomputeNormal) {
                                var p2 = Cartesian3.Cartesian3.fromArray(
                                    flatPositions,
                                    i + length,
                                    p2Scratch
                                );
                            ...
                            if (
                                Cartesian3.Cartesian3.equalsEpsilon(
                                    p1,
                                    position,
                                    Cartesian3.CesiumMath.EPSILON10
                                )
                            ) {
                        ...
                        if (vertexFormat.tangent || vertexFormat.bitangent) {
                            if (perPositionHeight) {
                                ...
                                scratchPerPosTangent = Cartesian3.Cartesian3.normalize(
                                    Matrix4.Matrix3.multiplyByVector(
                                        tangentRotationMatrix,
                                        scratchPerPosTangent,
                                        scratchPerPosTangent
                                    ),
                            ...
                            tangent = Cartesian3.Cartesian3.cross(
                                Cartesian3.Cartesian3.UNIT_Z,
                                normal,
                                tangent
                            );
        ...
        if (options.extrude && when.defined(options.offsetAttribute)) {
            ...
            if (options.offsetAttribute === GeometryOffsetAttribute.GeometryOffsetAttribute.TOP) {
                ...
                var offsetValue =
                    options.offsetAttribute === GeometryOffsetAttribute.GeometryOffsetAttribute.NONE
                        ? 0
                        : 1;
    ...
    function computeRectangle(positions, ellipsoid, arcType, granularity, result) {
        result = when.defaultValue(result, new Rectangle.Rectangle());
        ...
        var inverseChordLength =
            1.0 / Cartesian3.CesiumMath.chordLength(granularity, ellipsoid.maximumRadius);
        ...
        if (
            ...
            if (result.east > Cartesian3.CesiumMath.PI) {
                result.east = result.east - Cartesian3.CesiumMath.TWO_PI;
            }
            if (result.west > Cartesian3.CesiumMath.PI) {
                result.west = result.west - Cartesian3.CesiumMath.TWO_PI;
            }
        }
    ...
    function interpolateAndGrowRectangle(
        ...
        for (var i = 0; i < numPoints; i++) {
            ...
            var lonAdjusted =
                longitude >= 0 ? longitude : longitude + Cartesian3.CesiumMath.TWO_PI;
    ...
    function PolygonGeometry(options) {
        ...
        var granularity = when.defaultValue(
            options.granularity,
            Cartesian3.CesiumMath.RADIANS_PER_DEGREE
        );
    ...
    PolygonGeometry.createBorderLine = function(params){
        ...
        if(params.compressPlane === true){
            ...
            positionHigh = points.map( p=>{
                return new Cartesian3.Cartesian3(p.x, p.y, min+height);
            });
            positionLow = points.map( p=>{
                return new Cartesian3.Cartesian3(p.x, p.y, min);
            });
        }else {
            positionHigh = points.map( p=>{
                return new Cartesian3.Cartesian3(p.x, p.y, p.z+height);
            });
            positionLow = points.map( p=>{
                return new Cartesian3.Cartesian3(p.x, p.y, p.z);
            });
        }
        ...
        let boundingSphere = BoundingSphere.BoundingSphere.fromPoints(points);
    ...
    PolygonGeometry.fromPositionsPlaneMode = function(params){
        ...
        let indices = PolygonPipeline.earcut(position2D);
        if(params.compressPlane === true){
            // 压到一个平面上
            let min = Infinity;
            points.forEach( p=>{
    ...
    PolygonGeometry.createGeometry = function (polygonGeometry) {
        ...
        var extrude =
            ...
            !Cartesian3.CesiumMath.equalsEpsilon(
                height,
                extrudedHeight,
                0,
                Cartesian3.CesiumMath.EPSILON2
            );
        ...
        var boundingSphere = BoundingSphere.BoundingSphere.fromVertices(
