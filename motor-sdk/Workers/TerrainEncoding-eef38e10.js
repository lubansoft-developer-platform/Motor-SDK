...
define(['exports', './when-45f3d25d', './Check-34538dad', './Cartesian3-e69091b9', './Cartesian2-7a44370a', './BoundingSphere-bacc5cb6', './Matrix4-c65e6a1b', './Rectangle-c7d55cfa', './ComponentDatatype-86703c58', './AttributeCompression-1e177d5e'], function (exports, when, Check, Cartesian3, Cartesian2, BoundingSphere, Matrix4, Rectangle, ComponentDatatype, AttributeCompression) { 'use strict';
    ...
    /**
        ...
        function EllipsoidalOccluder(ellipsoid, cameraPosition) {
            ...
            this._cameraPosition = new Cartesian3.Cartesian3();
            this._cameraPositionInScaledSpace = new Cartesian3.Cartesian3();
        ...
        Object.defineProperties(EllipsoidalOccluder.prototype, {
            ...
            cameraPosition : {
                ...
                set : function(cameraPosition) {
                    ...
                    var vhMagnitudeSquared = Cartesian3.Cartesian3.magnitudeSquared(cv) - 1.0;

                    Cartesian3.Cartesian3.clone(cameraPosition, this._cameraPosition);
        ...
        var scratchCartesian = new Cartesian3.Cartesian3();
        ...
        var scratchCameraPositionInScaledSpaceShrunk = new Cartesian3.Cartesian3();
        ...
        EllipsoidalOccluder.prototype.computeHorizonCullingPointFromRectangle = function(rectangle, ellipsoid, result) {
            ...
            var positions = Rectangle.Rectangle.subsample(rectangle, ellipsoid, 0.0, subsampleScratch);
            var bs = BoundingSphere.BoundingSphere.fromPoints(positions);

            // If the bounding sphere center is too close to the center of the occluder, it doesn't make
            // sense to try to horizon cull it.
            if (Cartesian3.Cartesian3.magnitude(bs.center) < 0.1 * ellipsoid.minimumRadius) {
                return undefined;
            }
        ...
        var scratchEllipsoidShrunkRadii = new Cartesian3.Cartesian3();

        function getPossiblyShrunkEllipsoid(ellipsoid, minimumHeight, result) {
            if (when.defined(minimumHeight) && minimumHeight < 0.0 && ellipsoid.minimumRadius > -minimumHeight) {
                var ellipsoidShrunkRadii = Cartesian3.Cartesian3.fromElements(
        ...
        var positionScratch = new Cartesian3.Cartesian3();
        ...
        function isScaledSpacePointVisible(occludeeScaledSpacePosition, cameraPositionInScaledSpace, distanceToLimbInScaledSpaceSquared) {
            ...
            var vt = Cartesian3.Cartesian3.subtract(occludeeScaledSpacePosition, cv, scratchCartesian);
            var vtDotVc = -Cartesian3.Cartesian3.dot(vt, cv);
            // If vhMagnitudeSquared < 0 then we are below the surface of the ellipsoid and
            // in this case, set the culling plane to be on V.
            var isOccluded = vhMagnitudeSquared < 0 ? vtDotVc > 0 : (vtDotVc > vhMagnitudeSquared &&
                             vtDotVc * vtDotVc / Cartesian3.Cartesian3.magnitudeSquared(vt) > vhMagnitudeSquared);
            return !isOccluded;
        }

        var scaledSpaceScratch = new Cartesian3.Cartesian3();
        var directionScratch = new Cartesian3.Cartesian3();

        function computeMagnitude(ellipsoid, position, scaledSpaceDirectionToPoint) {
            var scaledSpacePosition = ellipsoid.transformPositionToScaledSpace(position, scaledSpaceScratch);
            var magnitudeSquared = Cartesian3.Cartesian3.magnitudeSquared(scaledSpacePosition);
            var magnitude = Math.sqrt(magnitudeSquared);
            var direction = Cartesian3.Cartesian3.divideByScalar(scaledSpacePosition, magnitude, directionScratch);

            // For the purpose of this computation, points below the ellipsoid are consider to be on it instead.
            magnitudeSquared = Math.max(1.0, magnitudeSquared);
            magnitude = Math.max(1.0, magnitude);

            var cosAlpha = Cartesian3.Cartesian3.dot(direction, scaledSpaceDirectionToPoint);
            var sinAlpha = Cartesian3.Cartesian3.magnitude(Cartesian3.Cartesian3.cross(direction, scaledSpaceDirectionToPoint, direction));
        ...
        function magnitudeToPoint(scaledSpaceDirectionToPoint, resultMagnitude, result) {
            ...
            return Cartesian3.Cartesian3.multiplyByScalar(scaledSpaceDirectionToPoint, resultMagnitude, result);
        }

        var directionToPointScratch = new Cartesian3.Cartesian3();

        function computeScaledSpaceDirectionToPoint(ellipsoid, directionToPoint) {
            if (Cartesian3.Cartesian3.equals(directionToPoint, Cartesian3.Cartesian3.ZERO)) {
                return directionToPoint;
            }

            ellipsoid.transformPositionToScaledSpace(directionToPoint, directionToPointScratch);
            return Cartesian3.Cartesian3.normalize(directionToPointScratch, directionToPointScratch);
        }
    ...
    var cartesian3Scratch = new Cartesian3.Cartesian3();
        var cartesian3DimScratch = new Cartesian3.Cartesian3();
        var cartesian2Scratch = new Cartesian2.Cartesian2();
        var matrix4Scratch = new Matrix4.Matrix4();
        var matrix4Scratch2 = new Matrix4.Matrix4();
        ...
        function TerrainEncoding(axisAlignedBoundingBox, minimumHeight, maximumHeight, fromENU, hasVertexNormals, hasWebMercatorT) {
            ...
            if (when.defined(axisAlignedBoundingBox) && when.defined(minimumHeight) && when.defined(maximumHeight) && when.defined(fromENU)) {
                ...
                var dimensions = Cartesian3.Cartesian3.subtract(maximum, minimum, cartesian3DimScratch);
                var hDim = maximumHeight - minimumHeight;
                var maxDim = Math.max(Cartesian3.Cartesian3.maximumComponent(dimensions), hDim);
                ...
                toENU = Matrix4.Matrix4.inverseTransformation(fromENU, new Matrix4.Matrix4());

                var translation = Cartesian3.Cartesian3.negate(minimum, cartesian3Scratch);
                Matrix4.Matrix4.multiply(Matrix4.Matrix4.fromTranslation(translation, matrix4Scratch), toENU, toENU);

                var scale = cartesian3Scratch;
                scale.x = 1.0 / dimensions.x;
                scale.y = 1.0 / dimensions.y;
                scale.z = 1.0 / dimensions.z;
                Matrix4.Matrix4.multiply(Matrix4.Matrix4.fromScale(scale, matrix4Scratch), toENU, toENU);

                matrix = Matrix4.Matrix4.clone(fromENU);
                Matrix4.Matrix4.setTranslation(matrix, Cartesian3.Cartesian3.ZERO, matrix);

                fromENU = Matrix4.Matrix4.clone(fromENU, new Matrix4.Matrix4());

                var translationMatrix = Matrix4.Matrix4.fromTranslation(minimum, matrix4Scratch);
                var scaleMatrix =  Matrix4.Matrix4.fromScale(dimensions, matrix4Scratch2);
                var st = Matrix4.Matrix4.multiply(translationMatrix, scaleMatrix,matrix4Scratch);

                Matrix4.Matrix4.multiply(fromENU, st, fromENU);
                Matrix4.Matrix4.multiply(matrix, st, matrix);
            }
        ...
        TerrainEncoding.prototype.encode = function(vertexBuffer, bufferIndex, position, uv, height, normalToPack, webMercatorT) {
            ...
            if (this.quantization === TerrainQuantization$1.BITS12) {
                position = Matrix4.Matrix4.multiplyByPoint(this.toScaledENU, position, cartesian3Scratch);

                position.x = Cartesian3.CesiumMath.clamp(position.x, 0.0, 1.0);
                position.y = Cartesian3.CesiumMath.clamp(position.y, 0.0, 1.0);
                position.z = Cartesian3.CesiumMath.clamp(position.z, 0.0, 1.0);

                var hDim = this.maximumHeight - this.minimumHeight;
                var h = Cartesian3.CesiumMath.clamp((height - this.minimumHeight) / hDim, 0.0, 1.0);
                ...
                Cartesian3.Cartesian3.subtract(position, this.center, cartesian3Scratch);
        ...
        TerrainEncoding.prototype.decodePosition = function(buffer, index, result) {
            ...
            if (this.quantization === TerrainQuantization$1.BITS12) {
                ...
                return Matrix4.Matrix4.multiplyByPoint(this.fromScaledENU, result, result);
            }

            result.x = buffer[index];
            result.y = buffer[index + 1];
            result.z = buffer[index + 2];
            return Cartesian3.Cartesian3.add(result, this.center, result);
        };
        ...
        TerrainEncoding.clone = function(encoding, result) {
            ...
            result.center = Cartesian3.Cartesian3.clone(encoding.center);
            result.toScaledENU = Matrix4.Matrix4.clone(encoding.toScaledENU);
            result.fromScaledENU = Matrix4.Matrix4.clone(encoding.fromScaledENU);
            result.matrix = Matrix4.Matrix4.clone(encoding.matrix);
...
//# sourceMappingURL=TerrainEncoding-eef38e10.js.map
