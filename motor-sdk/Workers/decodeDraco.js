...
define(['./when-45f3d25d', './Check-34538dad', './Cartesian3-e69091b9', './Cartesian2-7a44370a', './Matrix4-c65e6a1b', './RuntimeError-86da6af2', './WebGLConstants-3660bc8f', './ComponentDatatype-86703c58', './PrimitiveType-30fa6f85', './Transforms-7d49a8ab', './IndexDatatype-e52361bd', './createTaskProcessorWorker', './CreatePhysicalArray-d462d0a0', './materem-f017d2c3', './EmWrapperManager-a482472d', './EmLBDeal-8c054e74'], function (when, Check, Cartesian3, Cartesian2, Matrix4, RuntimeError, WebGLConstants, ComponentDatatype, PrimitiveType, Transforms, IndexDatatype, createTaskProcessorWorker, CreatePhysicalArray, materem, EmWrapperManager, EmLBDeal) { 'use strict';
    ...
    function decodeAttribute(dracoGeometry, dracoDecoder, dracoAttribute) {
        ...
        return {
            ...
            data: {
                ...
                byteStride:
                    ComponentDatatype.ComponentDatatype.getSizeInBytes(componentDatatype) *
                    numComponents,
    ...
    function decodePrimitive(parameters) {
        ...
        for (var attributeName in compressedAttributes) {
            if (compressedAttributes.hasOwnProperty(attributeName)) {
                ...
                if (
                    ...
                    var emLBDeal = new EmLBDeal.EmLBDeal();
                    ...
                    if (isInit) {
                        ...
                        if (parameters.projected) {
                            emLBDeal.computeProjToCartesianAry(typedArray, isYUp, Matrix4.Matrix4.fromArray(matrix));
                        } else {
                            emLBDeal.computeCartesianToProjAry(typedArray, isYUp, Matrix4.Matrix4.fromArray(matrix));
                        }
        ...
        var pPtAry = when.defined(attributeData["POSITION"])
            ? attributeData["POSITION"].array
            : undefined;
        var pBatchIdAry = when.defined(attributeData["_BATCHID"])
            ? attributeData["_BATCHID"].array
            : undefined;
        var pIndexAry = indexArray.typedArray;
        var pEdgeCheckAry;
        var physicalArray = CreatePhysicalArray.CreatePhysicalArray.createPhysicalArrayFromModel(
            physical,
            lbSpaMgr,
    ...
    function decode(parameters, transferableObjects) {
        if (
            when.defined(parameters.dracoType) &&
            parameters.dracoType === DracoType$1.PROJECTIVE_TRANSFORM
        ) {
            //非压缩模型，直接球面投影变换
        ...
        if (when.defined(parameters.primitive)) {
            if (
                when.defined(parameters.dracoType) &&
                parameters.dracoType === DracoType$1.MATER_COMPRESSION
            ) {
    ...
    function initWorker(dracoModule, wasmBinaryFileES6) {
        EmWrapperManager.EmWrapperManager.initWebAssembly(wasmBinaryFileES6).then(function() {
            physical = EmWrapperManager.emMod;
