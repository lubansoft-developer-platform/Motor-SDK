...
define(['exports', './when-45f3d25d', './Check-34538dad', './Cartesian3-e69091b9', './Cartesian2-7a44370a'], function (exports, when, Check, Cartesian3, Cartesian2) { 'use strict';
    ...
    function calculateM(ellipticity, major, latitude) {
        ...
        function calculateSigma(ellipticity, latitude) {
            if (ellipticity === 0.0) { // sphere
                return Math.log(Math.tan(0.5 * (Cartesian3.CesiumMath.PI_OVER_TWO + latitude)));
            }

            var eSinL = ellipticity * Math.sin(latitude);
            return Math.log(Math.tan(0.5 * (Cartesian3.CesiumMath.PI_OVER_TWO + latitude))) - (ellipticity / 2.0 * Math.log((1 + eSinL) / (1 - eSinL)));
        }
        ...
        function calculateHeading(ellipsoidRhumbLine, firstLongitude, firstLatitude, secondLongitude, secondLatitude) {
            ...
            return Math.atan2(Cartesian3.CesiumMath.negativePiToPi(secondLongitude - firstLongitude), sigma2 - sigma1);
        }
        ...
        function calculateArcLength(ellipsoidRhumbLine, major, minor, firstLongitude, firstLatitude, secondLongitude, secondLatitude) {
            ...
            if (Cartesian3.CesiumMath.equalsEpsilon(Math.abs(heading), Cartesian3.CesiumMath.PI_OVER_TWO, Cartesian3.CesiumMath.EPSILON8)) { //If heading is close to 90 degrees
                if (major === minor) {
                    distance = major * Math.cos(firstLatitude) * Cartesian3.CesiumMath.negativePiToPi(deltaLongitude);
                } else {
                    var sinPhi = Math.sin(firstLatitude);
                    distance = major * Math.cos(firstLatitude) * Cartesian3.CesiumMath.negativePiToPi(deltaLongitude) / Math.sqrt(1 - ellipsoidRhumbLine._ellipticitySquared * sinPhi * sinPhi);
                }
            } else {
        ...
        var scratchCart1 = new Cartesian3.Cartesian3();
        var scratchCart2 = new Cartesian3.Cartesian3();

        function computeProperties(ellipsoidRhumbLine, start, end, ellipsoid) {
            var firstCartesian = Cartesian3.Cartesian3.normalize(ellipsoid.cartographicToCartesian(start, scratchCart2), scratchCart1);
            var lastCartesian = Cartesian3.Cartesian3.normalize(ellipsoid.cartographicToCartesian(end, scratchCart2), scratchCart2);

            //>>includeStart('debug', pragmas.debug);
            Check.Check.typeOf.number.greaterThanOrEquals('value', Math.abs(Math.abs(Cartesian3.Cartesian3.angleBetween(firstCartesian, lastCartesian)) - Math.PI), 0.0125);
            //>>includeEnd('debug');
        ...
        function interpolateUsingSurfaceDistance(start, heading, distance, major, ellipticity, result)
        {
            ...
            if (Math.abs(Cartesian3.CesiumMath.PI_OVER_TWO - Math.abs(heading)) > Cartesian3.CesiumMath.EPSILON8) {
                ...
                longitude = Cartesian3.CesiumMath.negativePiToPi(start.longitude + deltaLongitude);
        ...
        EllipsoidRhumbLine.fromStartHeadingDistance = function(start, heading, distance, ellipsoid, result) {
            ...
            heading = Cartesian3.CesiumMath.negativePiToPi(heading);
        ...
        EllipsoidRhumbLine.prototype.findIntersectionWithLongitude = function(intersectionLongitude, result) {
            ...
            intersectionLongitude = Cartesian3.CesiumMath.negativePiToPi(intersectionLongitude);

            if (Cartesian3.CesiumMath.equalsEpsilon(Math.abs(intersectionLongitude), Math.PI, Cartesian3.CesiumMath.EPSILON14)) {
                intersectionLongitude = Cartesian3.CesiumMath.sign(start.longitude) * Math.PI;
            }
            ...
            if (Math.abs(Cartesian3.CesiumMath.PI_OVER_TWO - absHeading) <= Cartesian3.CesiumMath.EPSILON8) {
                result.longitude = intersectionLongitude;
                result.latitude = start.latitude;
                result.height = 0;
                return result;
            } else if (Cartesian3.CesiumMath.equalsEpsilon(Math.abs(Cartesian3.CesiumMath.PI_OVER_TWO - absHeading), Cartesian3.CesiumMath.PI_OVER_TWO, Cartesian3.CesiumMath.EPSILON8)) {
                if (Cartesian3.CesiumMath.equalsEpsilon(intersectionLongitude, start.longitude, Cartesian3.CesiumMath.EPSILON12)) {
                    return undefined;
                }

                result.longitude = intersectionLongitude;
                result.latitude = Cartesian3.CesiumMath.PI_OVER_TWO * Cartesian3.CesiumMath.sign(Cartesian3.CesiumMath.PI_OVER_TWO - heading);
                result.height = 0;
                return result;
            }
            ...
            var leftComponent = Math.tan(0.5 * (Cartesian3.CesiumMath.PI_OVER_TWO + phi1)) * Math.exp((intersectionLongitude - start.longitude) / Math.tan(heading));
            ...
            do {
                ...
                newPhi = 2 * Math.atan(leftComponent * Math.pow(numerator / denominator, ellipticity / 2)) - Cartesian3.CesiumMath.PI_OVER_TWO;
            } while (!Cartesian3.CesiumMath.equalsEpsilon(newPhi, phi, Cartesian3.CesiumMath.EPSILON12));
        ...
        EllipsoidRhumbLine.prototype.findIntersectionWithLatitude = function(intersectionLatitude, result) {
            ...
            if (Cartesian3.CesiumMath.equalsEpsilon(Math.abs(heading), Cartesian3.CesiumMath.PI_OVER_TWO, Cartesian3.CesiumMath.EPSILON8)) {
                return;
            }
            ...
            var longitude = Cartesian3.CesiumMath.negativePiToPi(start.longitude + deltaLongitude);
...
//# sourceMappingURL=EllipsoidRhumbLine-4bc7760a.js.map
