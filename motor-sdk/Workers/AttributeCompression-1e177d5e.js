...
define(['exports', './when-45f3d25d', './Check-34538dad', './Cartesian3-e69091b9', './Cartesian2-7a44370a'], function (exports, when, Check, Cartesian3, Cartesian2) { 'use strict';
    ...
    var RIGHT_SHIFT = 1.0 / 256.0;
        ...
        AttributeCompression.octEncodeInRange = function(vector, rangeMax, result) {
            ...
            var magSquared = Cartesian3.Cartesian3.magnitudeSquared(vector);
            if (Math.abs(magSquared - 1.0) > Cartesian3.CesiumMath.EPSILON6) {
            ...
            if (vector.z < 0) {
                var x = result.x;
                var y = result.y;
                result.x = (1.0 - Math.abs(y)) * Cartesian3.CesiumMath.signNotZero(x);
                result.y = (1.0 - Math.abs(x)) * Cartesian3.CesiumMath.signNotZero(y);
            }

            result.x = Cartesian3.CesiumMath.toSNorm(result.x, rangeMax);
            result.y = Cartesian3.CesiumMath.toSNorm(result.y, rangeMax);
        ...
        AttributeCompression.octDecodeInRange = function(x, y, rangeMax, result) {
            ...
            result.x = Cartesian3.CesiumMath.fromSNorm(x, rangeMax);
            result.y = Cartesian3.CesiumMath.fromSNorm(y, rangeMax);
            result.z = 1.0 - (Math.abs(result.x) + Math.abs(result.y));

            if (result.z < 0.0)
            {
                var oldVX = result.x;
                result.x = (1.0 - Math.abs(result.y)) * Cartesian3.CesiumMath.signNotZero(oldVX);
                result.y = (1.0 - Math.abs(oldVX)) * Cartesian3.CesiumMath.signNotZero(result.y);
            }
...
//# sourceMappingURL=AttributeCompression-1e177d5e.js.map
